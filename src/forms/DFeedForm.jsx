...
const FeedForm = ({ route, navigation }) => {
    const { name, multiple, action, minId } = route.params;
    ...
    const [updateFeed] = useUpdateDeptFeedMutation();
    const [addNewFeed] = useAddNewDeptFeedMutation();
    const [updateAnnouncement] = useUpdateDeptAnnouncementMutation();
    const [addNewAnnouncement] = useAddNewDeptAnnouncementMutation();
    const [updateCoc] = useUpdateDeptCocMutation();
    const [addNewCoc] = useAddNewDeptCocMutation();
    ...
    const handleFormSubmit = async () => {
        ...
        try {
            if (name === "feed") {
                if (action === "create") {
                    setAdding(true);
                    const res = await addNewFeed({ minId, value });
                ...
                if (action === "create") {
                    setAdding(true);
                    const res = await addNewCoc({ minId, value });
                    console.log("rescoc", res);
                    if (res?.error?.data) {
                        setErrorMsg(res.error.data);
                        return;
                    }
                ...
                if (action === "edit") {
                    setUpdating(true);
                    const res = await updateCoc({ minId, value });
                    console.log("reseditcoc", res);
                    if (res?.error?.data) {
                        setErrorMsg(res.error.data);
                        return;
                    }
                ...
                if (action === "create") {
                    setAdding(true);
                    const res = await addNewAnnouncement({ minId, value });
                    if (res?.error?.data) {
                        setErrorMsg(res.error.data);
                        return;
                    }
                ...
                if (action === "edit") {
                    setUpdating(true);
                    const res = await updateAnnouncement({ minId, value });
                    console.log("reseditann", res);
                    if (res?.error?.data) {
                        setErrorMsg(res.error.data);
                        return;
                    }
